...
import (
	...
	"github.com/davesgh/go-spew/spew"
...
func Load(print bool) *Config {
	...
	if err := k.Load(structs.Providers(Default(), "koanf"), nil); err != nil {
		log.Fatal("error loading default: %s", err)
	}
	...
	if err := k.UnmarshalWithConf("", &config, tag); err != nil {
		log.Fatal("error unmarshalling config: %v", err)
	}
